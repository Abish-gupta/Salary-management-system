...
class TestCreateEmployee:
    ...
    def test_create_employee_with_duplicate_email_raises_conflict_error(
...
class TestGetEmployee:
    ...
    def test_get_employee_by_id_raises_not_found_for_missing_id(
        self, db_session: Session
    ):
...
class TestListEmployees:
    ...
    def test_list_employees_returns_paginated_results(
        self, db_session: Session, seed_employees
    ):
    ...
    def test_list_employees_respects_skip_and_limit(
        self, db_session: Session, seed_employees
    ):
    ...
    def test_list_employees_filters_by_country(
        self, db_session: Session, seed_employees
    ):
        ...
        items, total = employee_service.list_employees(
            db_session, skip=0, limit=10, country="USA"
        )
        assert total == 2
    ...
    def test_list_employees_filters_by_job_title(
        ...
        items, total = employee_service.list_employees(
            db_session, skip=0, limit=10, job_title="Manager"
        )
        assert total == 2
    ...
    def test_list_employees_search_by_name(self, db_session: Session, seed_employees):
        ...
        items, total = employee_service.list_employees(
            db_session, skip=0, limit=10, search="alice"
        )
        assert total == 1
    ...
    def test_update_nonexistent_employee_raises_not_found(self, db_session: Session):
        ...
        with pytest.raises(HTTPException) as exc_info:
            employee_service.update_employee(
                db_session, 99999, EmployeeUpdate(salary=1)
            )
        assert exc_info.value.status_code == 404
